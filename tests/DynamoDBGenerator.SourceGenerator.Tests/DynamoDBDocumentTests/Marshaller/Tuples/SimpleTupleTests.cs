...
public partial class SimpleTupleTests : MarshalAsserter<(string FirstName, string LastName, string? EmailAddress)>
{
    ...
    private static ((string, string, string? ), Dictionary<string, AttributeValue>) Av((string FirstName, string LastName, string? EmailAddress) tuple)
    {
    ...
    protected override IEnumerable<((string FirstName, string LastName, string? EmailAddress) element, Dictionary<string, AttributeValue> attributeValues)> Arguments()
    {
    ...
    protected override (string FirstName, string LastName, string? EmailAddress) UnmarshallImplementation(Dictionary<string, AttributeValue> attributeValues)
    {
        return ValueTupleMarshaller.Unmarshall(attributeValues);
    }
    protected override Dictionary<string, AttributeValue> MarshallImplementation((string FirstName, string LastName, string? EmailAddress) element)
    {
